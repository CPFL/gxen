...
//
// added small modification for nvc0 project
//  rm boost dependency
//  mv to namespace nvc0
//
#ifndef HW_NVC0_NVC0_UNIQUE_PTR_H_
#define HW_NVC0_NVC0_UNIQUE_PTR_H_
#include "nvc0_platform.h"

#if defined(NVC0_COMPILER_MSVC_10) || defined(__GXX_EXPERIMENTAL_CXX0X__)
...
#else
#include "nvc0_type_traits.h"
#include "nvc0_if.h"
#include "nvc0_enable_if.h"
...
template <class T1, class T2, bool = std::is_empty<T2>::value>
class unique_ptr_storage
{
    T1 t1_;
    T2 t2_;

    typedef typename std::add_reference<T2>::type T2_reference;
    typedef typename std::add_reference<const T2>::type T2_const_reference;
...
struct default_delete
{
    ...
    {
        NVC0_STATIC_ASSERT(sizeof(T) > 0, type_is_invalid);
        delete ptr;
    }
};
...
private:
    ...
    typedef typename std::add_reference<deleter_type>::type deleter_reference;
    typedef typename std::add_reference<const deleter_type>::type deleter_const_reference;
...
public:
    ...
    unique_ptr()
        {
            NVC0_STATIC_ASSERT(!std::is_reference<deleter_type>::value, type_is_reference);
            NVC0_STATIC_ASSERT(!std::is_pointer<deleter_type>::value, type_is_pointer);
        }
    ...
    // TODO(Constellation) fix if_
    unique_ptr(pointer p, typename nvc0::if_<std::is_reference<D>,
                          volatile typename std::remove_reference<D>::type&, D>::type d)
        : ptr_(move(p), forward<D>(const_cast<typename std::add_reference<D>::type>(d))) {}
    ...
    typename std::add_reference<T>::type operator*() const {return *get();}
...
private:
    ...
    template <class U>
        unique_ptr(U, typename nvc0::if_<std::is_reference<D>,
                          volatile typename std::remove_reference<D>::type&, D>::type,
                          typename enable_if_c<detail_unique_ptr::is_convertible<U, pointer>::value>::type* = 0);
};
...
#endif
#endif  // HW_NVC0_NVC0_UNIQUE_PTR_H_
